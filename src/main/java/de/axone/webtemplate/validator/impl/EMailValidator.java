 ...
 * 
 * Thanks to: http://www.leshazlewood.com/?p=5
 * 
 * @author flo
 */
public class EMailValidator extends AbstractValidator<String> {

	private static final String NO_EMAIL = "VALIDATOR_NO_EMAIL";
	...
	// RFC 2822 3.2.2 Quoted characters: //single backslash followed by a text char 
	...
	// RFC 2822 3.2.5 Quoted strings: //noWsCtl and the rest of ASCII except the
	// doublequote and backslash characters: 
	...
	private static final String rfc1035DomainName = rfcLabel + "(\\."
			+ rfcLabel + ")*\\." + letter + "{2,6}";
	*/
	// RFC 2822 3.4 Address specification //domain text - non white space
	// controls and the rest of ASCII chars not including [, ], or \: 
	...
	private static final String domain = ALLOW_DOMAIN_LITERALS ? rfc2822Domain
			: rfc1035DomainName;
	*/
	private static final String domain = rfc2822Domain;
	...
	// now compile a pattern for efficient re-use: //if we're allowing quoted
	// identifiers or not: 
	...
	public String validate( String value, @Nullable Translator t ) {
		...
		if( VALID_PATTERN.matcher( value ).matches() ) return null;
